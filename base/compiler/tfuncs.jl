...
function isdefined_nothrow(argtypes::Array{Any, 1})
    if argtypes[1] ⊑ Type
        return (argtypes[2] ⊑ Symbol || argtypes[2] ⊑ Int)
    elseif argtypes[1] ⊑ Module
        return argtypes[2] ⊑ Symbol
    end
    return false
end
...
function fieldtype_nothrow(@nospecialize(s0), @nospecialize(name))
    ...
    if isa(s, Union)
        return _fieldtype_nothrow(s.a, name) && _fieldtype_nothrow(s.b, name)
    end
    return _fieldtype_nothrow(s, name)
end

function _fieldtype_nothrow(@nospecialize(s), @nospecialize(name))
    fld = name.val
    if isa(fld, Symbol)
        fld = fieldindex(s, fld, false)
    end
...
function _builtin_nothrow(@nospecialize(f), argtypes::Array{Any,1}, @nospecialize(rt))
    ...
    elseif f === Core.sizeof
        return sizeof_nothrow(argtypes)
