...
function *(transA::Transpose{<:Any,<:StridedMatrix{Ta}}, x::AbstractSparseVector{Tx}) where {Ta,Tx}
    @assert is_one_indexed(transA, x)
    A = transA.parent
    m, n = size(A)
